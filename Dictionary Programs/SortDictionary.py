...
dict = {'watermelon': 10, 'apple': 5, 'banana': 7, 'grapes': 3}
asc = {k: v for k, v in sorted(dict.items(), key = lambda item: item[0])}          #Sorts the dictionary by its keys in ascending order using sorted() with a lambda function as the key, and constructs a new dictionary with the sorted items.
print(asc)
...
from collections import OrderedDict
dict = {'watermelon': 10, 'apple': 5, 'banana': 7, 'grapes': 3}
asc = OrderedDict(sorted(dict.items(), key = lambda item: item[0]))          #Sorts the dictionary by its keys in ascending order using
#sorted() with a lambda function as the key, and constructs an OrderedDict to maintain the sorted order.
print(asc)


#Using for loops with sorted
dict = {'watermelon': 10, 'apple': 5, 'banana': 7, 'grapes': 3}
for k, v in sorted(dict.items(), key = lambda item: item[0]):          #Iterates through the sorted items of the dictionary by key using sorted() with a lambda function as the key.
    print((k, v), end = ' ')          #Prints each key-value pair in ascending order of keys.
...
k = list(dict.keys())          #Extracts the keys of the dictionary into a list.
v = list(dict.values())          #Extracts the values of the dictionary into a list.
index = np.argsort(k)          #Uses numpy's argsort() to get the indices that would sort the keys list in ascending order.
sorted_dict = {k[i]: v[i] for i in index}          #Constructs a new dictionary by iterating through the sorted indices and mapping the corresponding keys and values.
print(sorted_dict)          #Prints the new dictionary sorted by keys in ascending order.